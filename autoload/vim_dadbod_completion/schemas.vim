...
\ }
  let s:bigquery = {
        ...
        \ 'table_column_query': {table -> substitute('SELECT table_name, column_name FROM `INFORMATION_SCHEMA.COLUMNS` WHERE table_name={db_tbl_name}', '{db_tbl_name}', "'".table."'", '')},
        \ 'schemas_query': 'SELECT table_schema, table_name FROM `INFORMATION_SCHEMA.TABLES` GROUP BY table_schema, table_name',
        \ 'schemas_parser': function('s:map_and_filter', ['|']),
        \ 'quote': ['`', '`'],
